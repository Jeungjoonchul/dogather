	...
	<main>
		<div id="main">
			...
			<div class="section">
				...
				<div class="slidewrap">
					<ul class="slidelist">
						<li><a href="#"> <label for="slide3" class="s-left">&lt;</label>
								<img src="${cp }/resources/images/버거메인이미지1.png" alt="이미지1" /> <label
								for="slide2" class="s-right">&gt;</label>
						</a></li>
						<li><a href="#"> <label for="slide1" class="s-left">&lt;</label>
								<img src="${cp }/resources/images/bg.JPG" height="215.2px"
								alt="이미지2" /> <label for="slide3 class=s-right">&gt;</label>
						</a></li>
						<li><a href="#"> <label for="slide2" class="s-left">&lt;</label>
								<img src="" alt="이미지3" /> <label for="slide1" class="s-right">&gt;</label>
						</a></li>
					</ul>
				</div>
			</div>
			...
			<div id="dogather_list">
				...
				<div id="favorite">
					<div class="list_title">
						<a href="${cp}/dg/catalog.dg?criteria_key=dg_member_cnt"> <span
							class="purple">인기 </span><span>두ː게더</span>
						</a>
					</div>
					<div class="list_album">
						<c:choose>
							<c:when test="${hot_list.size()>0 and hot_list!=null }">
								<ul class="slides">
									<c:forEach items="${hot_list }" var="hl">
										<li><a href="${cp }/dg/detail.dg?dg_index=${hl.dg_index}">
												<div class="album">
													<c:choose>
														<c:when test="${hl.dg_banner_sys == null}">
															<c:choose>
																<c:when test="${hl.category_index eq 1 }">
																	<img src="${cp}/resources/images/default_health.png" />
																</c:when>
																<c:when test="${hl.category_index eq 2 }">
																	<img src="${cp}/resources/images/default_study.png" />
																</c:when>
																<c:when test="${hl.category_index eq 3 }">
																	<img
																		src="${cp}/resources/images/default_refinement.png" />
																</c:when>
																<c:when test="${hl.category_index eq 4 }">
														...
														<c:otherwise>
															<img
																src="${cp }/upload/images/dogather/banner/${hl.dg_banner_sys}" />
														</c:otherwise>
													</c:choose>
												</div> <span>${hl.dg_title }</span> <span class="dg_intro">
													<c:choose>
							...
							<c:otherwise>
								<div
									style="line-height: 300px; font-size: 49px; font-weight: bold">두개더가
									없습니다 ㅠㅠ</div>
							</c:otherwise>
						</c:choose>
					</div>
				...
				<div id="new">
					<div class="list_title">
						<a href="${cp}/dg/catalog.dg?criteria_key=dg_reg_date"> <span
							class="purple">최신 </span><span>두ː게더</span>
						</a>
					</div>
					<div class="list_album">
						<c:choose>
							<c:when test="${latest_list.size()>0 and latest_list!=null }">
								<ul class="slides">
									<c:forEach items="${latest_list }" var="ll">
										<li><a href="${cp }/dg/detail.dg?dg_index=${ll.dg_index}">
												<div class="album">
													<c:choose>
														<c:when test="${ll.dg_banner_sys == null}">
															<c:choose>
																<c:when test="${ll.category_index eq 1 }">
																	<img src="${cp}/resources/images/default_health.png" />
																</c:when>
																<c:when test="${ll.category_index eq 2 }">
																	<img src="${cp}/resources/images/default_study.png" />
																</c:when>
																<c:when test="${ll.category_index eq 3 }">
																	<img
																		src="${cp}/resources/images/default_refinement.png" />
																</c:when>
																<c:when test="${ll.category_index eq 4 }">
																	<img src="${cp}/resources/images/default_habit.png" />
																</c:when>
																<c:otherwise>
																	<img src="${cp}/resources/images/default_economy.png" />
														...
														<c:otherwise>
															<img
																src="${cp }/upload/images/dogather/banner/${ll.dg_banner_sys}" />
														</c:otherwise>
													</c:choose>
												</div> <span>${ll.dg_title }</span> <span class="dg_intro">
													<c:choose>
				...
				<div id="id="interest">
					<div class="list_title">
						<a href="${cp}/dg/catalog.dg?criteria_key=dg_hits"> <span
							class="purple">관심 </span><span>두ː게더</span>
						</a>
					</div>
					<div class="list_album">
						<c:choose>
							<c:when test="${mostView_list.size()>0 and mostView_list!=null }">
								<ul class="slides">
									<c:forEach items="${mostView_list }" var="ml">
										<li><a href="${cp }/dg/detail.dg?dg_index=${ml.dg_index}">
												<div class="album">
													<c:choose>
														<c:when test="${ml.dg_banner_sys == null}">
															<c:choose>
																<c:when test="${ml.category_index eq 1 }">
																	<img src="${cp}/resources/images/default_health.png" />
																</c:when>
																<c:when test="${ml.category_index eq 2 }">
																	<img src="${cp}/resources/images/default_study.png" />
																</c:when>
																<c:when test="${ml.category_index eq 3 }">
																	<img
																		src="${cp}/resources/images/default_refinement.png" />
																</c:when>
																<c:when test="${ml.category_index eq 4 }">
																	<img src="${cp}/resources/images/default_habit.png" />
																</c:when>
																<c:otherwise>
																	<img src="${cp}/resources/images/default_economy.png" />
														...
														<c:otherwise>
															<img
																src="${cp }/upload/images/dogather/banner/${ml.dg_banner_sys}" />
														</c:otherwise>
													</c:choose>
												</div> <span>${ml.dg_title }</span> <span class="dg_intro">
													<c:choose>
			...
			<div id="event_community">
				...
				<div id="event_border">
					<div class="event title">
						<a href="${cp }/board/event_board/post_list.bo"> <span
							class="purple">Event</span><span> 게시판</span>
						</a>
					</div>
							...
							<c:choose>
								<c:when test="${eb_list.size()>0 and eb_list!= null }">
									<c:forEach var="eb_list" items="${eb_list}">
										<fmt:parseDate var="date" value="${eb_list.b_reg_date }"
										...
										<li>
											...
											<div class="cap gray">
												<span>${eb_list.b_subject}</span>
											</div>

											<div class="tit">
												<a
													href="${cp}/board/event_board/post_view.bo?b_index=${eb_list.b_index}">
													${eb_list.b_title } </a>
											</div>
				...
				<div id="community_border">
					<div class="community title">
						<a href="${cp }/board/free_board/post_list.bo"> <span
							class="purple">자유 </span><span>게시판</span>
						</a>
					</div>
							...
							<c:choose>
								<c:when test="${fb_list.size()>0 and fb_list!= null }">
									<c:forEach var="fb_list" items="${fb_list}">
										<fmt:parseDate var="date" value="${fb_list.b_reg_date }"
											pattern="yyyy-MM-dd HH:mm:ss" />
										<c:set var="date">
											<fmt:formatDate value="${date }" pattern="yyyyMMdd" />
										</c:set>
										<li>
											...
											<div class="cap gray">
												<span>${fb_list.b_subject}</span>
											</div>

											<div class="tit">
												<a
													href="${cp}/board/free_board/post_view.bo?b_index=${fb_list.b_index}">
													${fb_list.b_title } </a>
											</div>
